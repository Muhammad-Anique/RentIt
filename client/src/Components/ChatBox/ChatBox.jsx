...
function ConversationComponent(props) {
  ...
  return (
    <div  onClick={()=>{props.Click(props.conversation.conversation_id); props.openConv()}} className={`flex flex-row w-full py-4 px-[25px] h-[100px] cursor-pointer gap-4 items-center hover:bg-[#e9f9fa] ${props.state.isActive ? 'bg-[#e9f9fa]' : ''}   `}>
...
function ChatBox(props) {
  ...
  const [containsBadLanguage, setContainsBadLanguage] = useState(false);
  ...
  useEffect(()=>{
    console.log("Set Is conve", isConvOpen)
  },[isConvOpen])
  ...
  return (
    <div className='w-full lg:w-[82vw] h-full bg-gray-100 p-[30px] flex flex-col gap-5'>
    <div className='flex flex-row justify-between items-center relative'>
      <div className={`bg-red-500 text-white ${bannerIsClosed ? ( containsBadLanguage ? 'flex' : 'hidden') : ('flex')} flex-row justify-between items-center gap-3 w-full h-[50px] rounded-md px-5 py-3 font-medium absolute z-50`}>
        <div className='flex-row flex gap-2 items-center'>
        <IoIosWarning className='text-yellow-200' size={30}/>
       {
        containsBadLanguage ? ( <p> {t("Warning: This text contains inappropriate language")}</p>) :( <p> {t("Note: Don't share your personal details if you dont trust the other person.")}</p>)
       }
        </div>
       
        <button onClick={()=>{setBannerIsClosed(!bannerIsClosed)}}><IoCloseSharp size={30} /></button>

      </div>
    <h1 className='text-2xl font-bold '>Chat</h1>
    </div>
    <div className={`h-[95%]  w-full flex  flex-row justify-between flex-wrap gap-2`}>
        <div name="ContactBar" className={`bg-white w-full  lg:w-[30%] ${isConvOpen ? ('hidden lg:flex') : ('flex')} h-full rounded-xl shadow-md overflow-y-auto`}>  
    ...
    (
      <div className={`bg-[#ffffff] w-full lg:w-[68%] ${isConvOpen ? ('flex') : ('hidden lg:flex')} h-[100%] rounded-lg shadow-md  flex flex-col`}>
      <div className='w-full h-[12%] border-b-2 border-b-gray-200'>
        <div className='flex flex-row w-full p-2 px-[25px] h-full  gap-4 items-center '>
          <div onClick={()=>{setIsConvOpen(!isConvOpen)}} className='cursor-pointer hover:text-primary lg:hidden'>
          <IoIosArrowBack />
          </div>
            <div>
      ...
      <div className='w-full h-[20%] border-t-2 border-t-gray-100 flex flex-row items-center justify-center gap-3 p-[20px]'>
       <textarea value={typedMessage} onChange={(e)=>{setTypedMessage(e.target.value);   const containsBadWord = filter.isProfane(e.target.value);
    setContainsBadLanguage(containsBadWord);}}  name=""  id=""  className='w-[90%] h-full rounded-lg border-2 border-gray-200 p-3' cols="30" rows="10" placeholder='Type...'></textarea>
